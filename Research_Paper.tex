...
$d \in \{2, 4, 6, 8\}$. The brute-force solver, which explores all
possible game states without memory, proves tractable only for small
configurations ($N=3$, $d\le 4$) and times out on larger instances due
to $O(b^d)$ exponential growth. In contrast, the DP memoized solver
leverages a transposition table that caches repeated board states,
achieving cache hit rates of up to 74\% and completing all
configurations up to $N=5$, $d=6$. We analyze each method's
implementation, complexity, and the cache hit rate as the key metric
explaining their divergent scalability.
\end{abstract}
...
\begin{itemize}
    ...
    \item An empirical evaluation quantifying the scalability gap,
          including cache hit rates up to 74\%.
...
\end{itemize}
Each configuration is executed once per solver; a timeout of 30 seconds
is enforced.
...
configuration. In practice, cache hit rates of 49--74\% are observed
...
\hline
Configs Completed & 2 of 12 & 10 of 12 \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:cachehit} reports the DP solver’s cache hit rates. The
hit rate grows from 0\% at $d=2$ to over 74\% at $d=8$, illustrating
...
$3{\times}3$ & 2 &        81 &         0 & 0.0\% \\
\hline
$3{\times}3$ & 4 &     2,097 &     1,008 & 48.1\% \\
\hline
$3{\times}3$ & 6 &    10,437 &     6,568 & 62.9\% \\
\hline
$3{\times}3$ & 8 &    15,945 &    10,546 & 66.1\% \\
\hline
$4{\times}4$ & 4 &    25,456 &    12,600 & 49.5\% \\
\hline
$4{\times}4$ & 6 &   636,976 &   420,280 & 66.0\% \\
\hline
$4{\times}4$ & 8 & 5,822,376 & 4,309,330 & 74.0\% \\
\hline
$5{\times}5$ & 4 &   166,225 &    82,800 & 49.8\% \\
\hline
$5{\times}5$ & 6 & 12,386,125 & 8,229,400 & 66.5\% \\
\hline
\end{tabular}
\end{table}

At $N = 3$, $d = 4$---the only configuration both solvers complete---
brute force explores 3,609 nodes versus 2,097 for DP, a reduction of
$1.72{\times}$. At $d = 6$, brute force times out while DP completes
in 9.40\,ms with 10,437 nodes. At $N = 4$, $d = 8$, DP explores
5,822,376 nodes with a 74\% hit rate, completing in 4,513\,ms.
...
exponentially and becomes impractical beyond the smallest configurations,
while the DP memoized solver handles significantly larger instances by
reusing computations. The cache hit rate is the critical factor: at
$d=2$ (0\% hits) both solvers behave similarly; as depth increases,
the hit rate approaches 74\%, reducing the effective node count by a
factor of $1/(1-\text{hit rate})$. For $N=4, d=8$, the memoized solver
explores 5.8M nodes but avoids 4.3M recomputations, yielding a feasible
search where brute force would exceed $b^d$ bounds.
...
reductions in search space, completing 10 of 12 benchmark configurations
that brute force could not. The cache hit rate, reaching 74\% at
$d=8$, quantifies the benefit of memoization. The work underscores the
importance of dynamic programming techniques in adversarial search and
provides a clear, measurable case study suitable for AI education and
algorithm design.
